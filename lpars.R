...
"plot.pairs"=function(A,txt="",label,prior,mome1,mome2)
{
	...
	if(prior[ind]>0)
	{
		...
		if(prior[ind]==2) B[,ind]=sqrt(1/rgamma(1000,mome1[ind],mome2[ind]))
...
LMC=read.psv("sca.psv")
LMC=cbind(exp(LMC[,1:4]),LMC[,5:6],sqrt(1/exp(LMC[,7])),LMC[,8])

lbs=c("ro","cr","rbar","fbar","ahat","rhat","tdev","rho")
plot.pairs(LMC,label=lbs,prior=c(0,0,0,0,0,0,0,0),mome1=c(0,0,0,0,0,0,0,0),mome2=c(0,0,0,0,0,0,0,0))
